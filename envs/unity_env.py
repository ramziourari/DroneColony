...
class Unity3DEnv(MultiAgentEnv):
    ...
    def get_policy_configs_for_game(
        ...
        obs_spaces = {
             "Game_MultiDroneDenseContinuous_neu": Box(float("-inf"), float("inf"), (10,)),  # 7 + 4 * (#drones -1)
        }
        action_spaces = {
            "Game_MultiDroneDenseContinuous_neu": Box(float("-inf"), float("inf"), (3,)),
        }
...
'''
        this can be useful when trying different groups/agents simultaneously
         
        if game_name == "Game_MultiDroneDenseContinuous_neu":
            policies = {
                "NavigationBrain": (None, obs_spaces["NavigationBrain"], action_spaces["NavigationBrain"], {}),
                "CollisionBrain": (None, obs_spaces["CollisionBrain"], action_spaces["CollisionBrain"], {}),
            }

            def policy_mapping_fn(agent_id):
                return "NavigationBrain" if "NavigationBrain" in agent_id else "CollisionBrain"
'''